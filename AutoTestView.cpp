...
{
	QString versions;
	versions.append(QString("%1\n").arg(Utility::buildDateTime(QStringLiteral("编译时间:"))));
...
bool AutoTestView::eventFilter(QObject* watched, QEvent* event)
{
	...
	if (stopevnt) {
		QSPMessageBox::warning(this, WARN_STR, QStringLiteral("正在测试中，请先停止测试"), QMessageBox::Yes);
	}
...
void AutoTestView::on_test_start_pb_clicked()
{
	...
	if (TestcaseBase::get_instance()->InterfaceidSetable()) {
		QSPMessageBox::warning(NULL,WARN_STR,QStringLiteral("请先连接设备"));
		return;
	}
	auto packet = new MessageFVBase;
